...
- Чистый рестайл: переносим court-разметку и `_base`-классы из соответствующих `sources/NNN-*/index.html`, НЕ трогая Server Actions, поля форм, валидацию, гейты ролей/readOnly, бизнес-логику и схему БД. v2.0-поведение записи сохраняется.
- **Единственное допустимое отклонение от «no backend» (autonomous-решение по vскрытым data-gaps):** READ-ONLY запросы в Server Components поверх существующих моделей — там, где дизайн-экран требует данные, которые v2.0-заглушка не читала (дашборд «Мои турниры»; список — фасет-`where` + capacity-счётчик). БЕЗ миграций схемы, БЕЗ новых/изменённых Server Actions, БЕЗ записи, БЕЗ изменения бизнес-логики. Это минимум для рендера Court и совпадает с паттерном «Server Components for reads». Если объём читающего слоя для дашборда окажется большим — планировщик вправе сузить до доступного и пометить остаток deferred (не выдумывать write-логику).
...
- **DATA GAP (вскрыт pattern-mapper):** v2.0-список рендерит только name/size/status; `listTournaments` не отдаёт счётчик регистраций для capacity «X/size». Поля format/level/kind(вид)/price/дата/место уже есть в модели Tournament — это чистый рендер существующих колонок. Дополнения (READ-ONLY, без схемы/миграций/записи): (1) расширить `where` для фасетов format/level/mode; (2) read-only агрегат-счётчик регистраций для capacity-progress. Если capacity-счётчик нетривиален (count по Pair + TournamentPlayer на формат) — на усмотрение планировщика показать «X/size» когда дёшево, иначе деградировать до size без дроби (не блокер UI-04: success-критерий требует layout + filter-поповер, не обязательно дробь). Поле «место»/venue — проверить наличие в схеме; нет → опустить.
...
- Каждый турнир — `.tcard` (2-col) с **ролью игрока** («пара с **Имя**» + аватар партнёра / «Одиночный зачёт»|«Одиночная заявка»), badge статуса, format·вид·место, и состоянием-зависимым правым блоком+CTA: active → round-progress «3/7» + primary «К текущему раунду»; upcoming → ghost «Открыть»; finished → `.place` (medal + «N место») + ghost «Результаты». Левый accent-bar `::before`: active→`--primary`, upcoming→`--accent`, finished→`--text-faint`. CTA ведут на существующие роуты.
- **DATA GAP (вскрыт pattern-mapper):** v2.0-дашборд — только приветствие; запроса «мои турниры» (участие игрока по 4 форматам + роль/партнёр + прогресс раундов) НЕ существует. Решение (autonomous, см. ниже): добавить **READ-ONLY** запрос в Server Component поверх СУЩЕСТВУЮЩИХ моделей (Pair player1/player2 для playoff/round_robin; TournamentPlayer для americano/mexicano/singles; Round для прогресса) — без миграций схемы, без новых Server Actions, без записи/изменения бизнес-логики. Это минимум данных для рендера Court-дизайна и согласуется с паттерном проекта «Server Components for reads».
