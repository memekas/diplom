...
**Goal:** перевести все экраны построенного фронтенда (v2.0) с базовой монохромной оболочки на зафиксированный дизайн-язык **Court** — без изменения функциональности, данных и backend.

**Target features (поверхности рестайла):**
- Дизайн-фундамент — тема Court + токены + компонентный слой в реальном Tailwind 4 (`globals.css`), container-query responsive.
- Auth (вход / регистрация).
- Browse — список турниров (collapsed-фильтры, карточки на телефоне), ЛК-дашборд, профиль.
- Tournament pages — страница турнира (programme + регламент + стартовый лист), создание турнира (условная форма по формату), сетка плей-офф (set-tally + games-on-hover, без счёта финала), форматные страницы (круговой / американо-мексикано: расписание + таблицы).

**Источник дизайна:** skill `sketch-findings-diplom` + `.planning/sketches/` (9 экранов, тема Court). Новых фич нет; стек неизменен (Next 16 + Tailwind 4, без UI-библиотек).
