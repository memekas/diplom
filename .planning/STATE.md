...
- [Phase ?]: 14-03: rotation uses «Ожидает счёта» (no target field, no «До N» pill); win/lose+.dim are display-only from stored points
...
| Visual UAT | Phase 13 экраны аккаунта/обзора: визуальная проверка auth таб-карточки, профиля (idcard+форма+changed-dot), списка (dense→card reflow + filter-поповер), дашборда (секции+роль-карточки) на десктопе и ≤375px (13-UAT.md). Код 4/4 верифицирован, гэпов нет. Провести в совместном визуальном UAT v3.0 после Phase 14. | Deferred | Phase 13 |
| Tech debt | Phase 13 дашборд: playoff round-progress «N/M» и finished-card `.place`/медаль не выводятся — final standings нельзя получить read-only без вызова движков форматов; вне рамок read-only рестайла v3.0. Если нужно — отдельная backend-задача. | Deferred | Phase 13 |
